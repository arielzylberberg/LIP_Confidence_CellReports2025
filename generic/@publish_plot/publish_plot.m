    ...
    methods
        function obj = publish_plot(n_filas,n_col,varargin)
            ...
            elseif obj.data.n_filas==1 && obj.data.n_col==3
                set(gcf,'Position',[297  297  902  246]);
            end
...
%             MarkerSize = [];
            ...
            for i=1:length(varargin)
                if isequal(varargin{i},'presentation')
                    ...
                    LineWidthAxes = 2;
                    LineWidthPlot = 2.5;
                    MarkerSize = 2*[15, 7]; % for '.' and 'o' respectively
            ...
            if not(isempty(MarkerSize))
                for j=1:length(obj.h_ax)
                    ...
                    for k=1:length(ch_line)
                        if isequal(ch_line(k).Marker,'o') || isequal(ch_line(k).Marker,'square')
...
%                     set(ch_errorbar,'MarkerSize',MarkerSize);
                    for k=1:length(ch_errorbar)
                        if isequal(ch_errorbar(k).Marker,'o') || isequal(ch_errorbar(k).Marker,'square')
