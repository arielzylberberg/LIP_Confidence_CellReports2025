function [p,lineas,out] = plot_lock_on_off_bigSbigT(s,smooth_win_t,conditions,filter,doPlot,...
    legend_str, varargin)
...
% tstep = smooth_win_t/5; % was equal to dt
tstep = dt;
[Son,ti_on] = bigSbigT(s.g_on,dt,tstep,smooth_win_t,conditions,filter,min_num_bins);
ton = s.t_on(ti_on);
...
% tstep = smooth_win_t/5; % was equal to dt
tstep = dt;
[Soff,ti_off] = bigSbigT(s.g_off,dt,tstep,smooth_win_t,conditions,filter,min_num_bins);
toff = s.t_off(ti_off);
